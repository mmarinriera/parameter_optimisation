...
__global__ void update_protrusions(float x_ratio, float y_ratio, const int n_cells,
...
int main(int argc, char const* argv[])
{
    ...
    std::string output_tag = codename + "_" + param1 + "_" + param2;
    float x_ratio = std::stof(param1);
    float y_ratio = x_ratio + std::stof(param2) * (1 - x_ratio);
    ...
    for (auto time_step = 0; time_step <= n_time_steps; time_step++) {
        ...
        update_protrusions<<<(protrusions.get_d_n() + 32 - 1) / 32, 32>>>(
            x_ratio, y_ratio, sphere.get_d_n(), grid.d_grid, sphere.d_X,
